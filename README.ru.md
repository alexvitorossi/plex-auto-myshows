...
Параллельно при старте и раз в сутки делается catch-up по `Plex.history()` — серии, досмотренные пока контейнер лежал, отметятся при ближайшем запуске. Применяется тот же порог, что и в реалтайме, поэтому частично проигранные серии из истории пропускаются.
